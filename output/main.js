...
var et = (c, u, h, A) => {
  if (u && typeof u == "object" || typeof u == "function") for (let x of Xe(u)) !Ye.call(c, x) && x !== h && Le(c, x, {
    get: () => u[x],
    enumerable: !(A = Ze(u, x)) || A.enumerable
  });
  return c;
};
var Ve = (c, u, h) => (h = c != null ? Ge(Qe(c)) : {}, et(u || !c || !c.__esModule ? Le(h, "default", {
  value: c,
  enumerable: true
}) : h, c));
var G = class {
...
`)) this.add(h);
  ...
  add(u) {
    if (this.rawRules.push(u), u = u.trim(), u = u.replace(/[\r\n]/g, ""), u.length <= 3 || u[0] === "!") return;
    let h = {
      hasStart: false,
      hasEnd: false,
      rule: u,
    ...
    if (u[0] === "|" && u[1] === "|") {
      ...
      let E = u.slice(2);
      h.text = Ue(E);
    } else u[0] === "|" ? (h.hasStart = true, h.text = u.slice(1)) : u[u.length - 1] === "|" && (h.hasEnd = true, h.text = u.slice(0, -1));
    let A = h.text.lastIndexOf("$");
    if (A >= 0) {
      h.text = h.text.slice(0, A);
      return;
    }
    if (h.text[0] === "/" && h.text[h.text.length - 1] === "/" || h.text.length <= 3) return;
    let x = h.text.split(/\*+/).filter(E => E);

    for (let C of x) {
      if (O.length > 0) for (let T = 0; T < O.length; T++) {
        let I = O[T];
        if (I === "") continue;
        let q = O[T - 1];
        let L = O[T + 1];
...
function tt(c, u, h) {
  let A = c.items;
  c.domain && (h.domainUrl ? u = h.domainUrl : (u = Ue(u), h.domainUrl = u));
  let x = -1;
  for (let E = 0; E < A.length; E++) {
    let C = A[E];
    let O = u.indexOf(C.text, x + 1);
    if (O <= x || (x = O, c.hasStart && E === 0 && O !== 0)) return false;
    if (c.hasEnd && E === A.length - 1) {
      let T = u.length - C.text.length;
      if (O !== T) return false;
    }
    if (C.before && !We(u[x - 1])) return false;
    if (C.after) {
      let T = x + C.text.length;
      if (u[T] !== undefined && !We(u[T])) return false;
    }
  }
  return true;
}
...
function Ue(c) {
  return c.indexOf("https://") === 0 && (c = c.slice(8)), c.indexOf("http://") === 0 && (c = c.slice(7)), c.indexOf("www.") === 0 && (c = c.slice(4)), c;
}
var i = require("electron");
var g = Ve(require("fs"));
var D = Ve(require("path"));
var Ne = require("url");
function qe(c, u) {
...
var nt = process.versions.electron;
var rt = parseInt(nt.split(".")[0]);
function j(c, u) {
  return c ? u() : [];
}
function je(c) {
...
function He(c, u, h) {
  let {
      editFlags: A,
      misspelledWord: x,
      dictionarySuggestions: E
    } = h;

  let C = h.selectionText.trim();
  let O = C.length > 0;
  let T = !!h.linkURL;
  let I = M => A[`can${M}`] && O;
  let q = h.isEditable || O;
  ...
  let L = [...j(v, () => [{
    ...
    label: "Cut",
    role: I("Cut") ? "cut" : undefined,
    enabled: I("Cut"),
    visible: h.isEditable
  }, {
    accelerator: "CmdOrCtrl+C",
    label: "Copy",
    role: I("Copy") ? "copy" : undefined,
    enabled: I("Copy"),
    visible: h.isEditable || O
  }, {
    accelerator: "CmdOrCtrl+V",
    label: "Paste",
    role: A.canPaste ? "paste" : undefined,
    enabled: A.canPaste,
    visible: h.isEditable
  }, {
    accelerator: "CmdOrCtrl+Shift+V",
    label: "Paste as text",
    role: A.canPaste ? "pasteAndMatchStyle" : undefined,
    enabled: A.canPaste,
    visible: h.isEditable
  }];
  ...
  if (x && x.length >= 1) {
    let M = [];
    E && E.length > 0 ? E.slice(0, 5).forEach(z => {
      M.push({
        label: z,
        click: () => {
    ...
    M.push({
      ...
      click: () => {
        u.session.addWordToSpellCheckerDictionary(x);
        u.replaceMisspelling(x);
      }
    });
    M.push({
      type: "separator"
  ...
  q && i.Menu.buildFromTemplate(L).popup({
    window: c
  });
}
function U(c, u) {
...
module.exports = (c, u, h) => {
  if (rt < 18) {
    i.dialog.showErrorBox("Manual update required", "This version of Obsidian is no longer supported. Please download and install the latest version from https://obsidian.md");
  ...
  for (let e of process.argv) e.startsWith("--enable-features=") && (A = A.concat(e.substring(18).split(",").map(o => o.trim())));
  i.app.commandLine.appendSwitch("enable-features", A.join(","));
  process.on("uncaughtException", e => {
    if (console.error("Uncaught Exception", e), e.message.includes("Render frame was disposed before WebFrameMain could be accessed")) return;
    let l = `Uncaught Exception:
` + (e.stack ? e.stack : `${e.name}: ${e.message}`);
    i.dialog.showErrorBox("A JavaScript error occurred in the main process", l);
  });
  let x = "";
  let E = false;
  u.on("update-manual-required", () => x = "update-manual-required");
  u.on("update-downloaded", () => x = "update-downloaded");
  u.on("check-start", () => E = true);
  u.on("check-end", () => E = false);
  ...
  async function Z() {
    ...
    for (let d of a) {
      ...
      try {
        ...
        let f = (new Date().getTime() - s.mtime.getTime()) / 86400000 /* 864e5 */;
        r = o === 0 ? false : f >= o;
      } catch (s) {}
      if (!r) {
        let s = D.join(e, D.basename(d));
        let f = await g.promises.readFile(s, "utf8");
        l.addList(f);
        continue;
      }
      ...
      try {
        ...
        await g.promises.writeFile(n, f);
        l.addList(f);
      } catch (s) {
    ...
    o !== 0 && (o = Math.min(o, 24), ne = setTimeout(Z, o * 86400000 /* 864e5 */));
  }
  function ze(e) {
    try {
      return g.accessSync(e, g.constants.R_OK | g.constants.W_OK), true;
    } catch (o) {
      return false;
    }
  }
  ...
  let re = (() => {
      ...
      try {
        if (g.existsSync(e)) return JSON.parse(g.readFileSync(e, "utf8")).version;
      } catch (o) {}
  ...
  async function X(e, o, l, a) {
    ...
    let t = d ? d[1].toLowerCase() : "";
    if (!t || t === xe || t === "about") return;
    if (t !== "http" && t !== "https" && t !== "obsidian" && !(b.openSchemes && b.openSchemes[t])) {
...
Link: ` + qe(o, 200),
      ...
      t && s.checkboxChecked && (b.openSchemes = b.openSchemes || {}, b.openSchemes[t] = true, k());
    }
    if (t === "http" || t === "https") {
      let s = "tab";
      let f = false;
      l === "new-window" ? (s = "window", f = true) : a === "split" ? (s = "split", f = true) : l === "background-tab" ? (s = "tab", f = false) : l === "foreground-tab" && (s = "tab", f = true);
    ...
    if (!o.startsWith(ie)) return console.log("Opening URL: " + o), i.shell.openExternal(o);
    let n = o.lastIndexOf("#");
    let r = "";
    n !== -1 && (r = o.substr(n), o = o.substr(0, n));
    o = decodeURIComponent(o.substr(ie.length));
    o = D.normalize(o) + r;
    !(_e(o) && (await i.dialog.showMessageBox(e, {
...
Location: ` + o,
    ...
    })).response !== 0) && (console.log("Opening file: " + o), be(o));
  }
  function be(e) {
    !N && !v ? i.shell.openExternal((0, Ne.pathToFileURL)(e).href) : i.shell.openPath(e);
  }
  function ye(e) {
    if (i.remote) try {
      i.remote.enable(e);
  ...
  function De(e, o) {
    ...
    l.on("will-navigate", (t, n) => {
      n.indexOf(Ae) !== 0 && (t.preventDefault(), n.indexOf(V) !== 0 && X(e, n));
    });
    N && e.on("app-command", (t, n) => {
      n === "browser-backward" ? l.executeJavaScript("history.back()") : n === "browser-forward" && l.executeJavaScript("history.forward()");
    });
    e.on("swipe", (t, n) => {
      n === "left" ? l.executeJavaScript("history.back()") : n === "right" && l.executeJavaScript("history.forward()");
    });
    e.on("focus", () => {
    ...
    e.on("always-on-top-changed", () => {
      e === i.BrowserWindow.getFocusedWindow() && pe(J(e), "window-always-on-top", {
        checked: e.isAlwaysOnTop()
      });
    });
    e.on("maximize", () => {
      e.isAlwaysOnTop() && e.setAlwaysOnTop(false);
    });
    e.on("enter-full-screen", () => {
      e.isAlwaysOnTop() && e.setAlwaysOnTop(false);
    });
    let a = () => e.webContents.executeJavaScript("window.dispatchEvent(new Event('focuschange'));");
    e.on("focus", a);
    e.on("blur", a);
    let d = () => l.executeJavaScript("window.dispatchEvent(new Event('fullscreenchange'));");
    e.on("enter-full-screen", d);
    e.on("leave-full-screen", d);
  }
  function we(e) {
    e.isSecured || (e.isSecured = true, e.setWindowOpenHandler(o => {
      if (o.url === "about:blank" && o.features && o.features.startsWith("popup")) {
        let l = o.features.split(",");
        let a = {};
        for (let d of l) {
          let [t, n] = d.split("=");
          (t === "x" || t === "y" || t === "width" || t === "height") && (a[t] = parseInt(n));
        }
        return {
          action: "allow",
      ...
      delete l.preloadURL;
      l.sandbox = true;
      l.nodeIntegration = false;
      l.nodeIntegrationInWorker = false;
      l.nodeIntegrationInSubFrames = false;
      l.webSecurity = true;
      l.plugins = false;
      l.experimentalFeatures = false;
      l.webviewTag = false;
  ...
  function Fe(e) {
    ...
    let l = false;
    if (e.x !== undefined && e.y !== undefined && e.width !== undefined && e.height !== undefined) try {
      for (let a of i.screen.getAllDisplays()) {
        let d = a.workArea;
        if (e.x < d.x + d.width - 2 && e.x + e.width > d.x + 2 && e.y < d.y + d.height - 2 && e.y + e.height > d.y + 2) {
          l = true;
          break;
        }
      }
      ...
      console.error(a);
    } else e.x === undefined && e.y === undefined && e.width !== undefined && e.height !== undefined && (l = true);
    return l && (o.x = e.x, o.y = e.y, o.width = e.width, o.height = e.height), o.width < 300 && (o.width = 300), o.height < 200 && (o.height = 200), o;
  }
  let Je = D.join(O, "Obsidian Vault");
  let Ce = D.join(C, "Obsidian Sandbox");
  let Ke = D.join(C, "Obsidian Help");
  let xe = "app";
  let oe = xe + "://";
  let V = oe + "obsidian.md/";
  let se = oe + je(36) + "/";
  let Ae = V + "index.html";
  let Ee = ["https://easylist.to/easylist/easylist.txt", "https://easylist.to/easylist/easyprivacy.txt"];
  let Se = 4;
  let Oe = ["clipboard-read", "clipboard-sanitized-write"];
  let b = z("obsidian");
  (!b || typeof b != "object") && (b = {});
  let F = b.vaults || {};
  $e();
  for (let e in F) {
    ...
    (!o.path || o.path === Ke || !g.existsSync(o.path)) && (delete F[e], ge(e));
  }
  b.vaults = F;
  b.insider && u.emit("insider", true);
  let Q = b.frame === "native";
  let le = Q ? "default" : "hidden";
  if (b.updateDisabled && (u.emit("disable", true), console.log("Updates disabled.")), b.disableGpu && !i.app.isReady()) try {
  ...
  function ve(e, o) {
    let l = F[e];
    l && (o ? l.open = true : delete l.open, k());
  }
  async function $e() {
  ...
  function Be(e, o = false) {
    ...
    for (let n = e.length - 1; n >= 0; n--) {
      let r = e[n];
      (r.label === "&Window" || r.label === "&Help") && (l.push(r), e.splice(n, 1));
    }
    function a(n) {
      n.forEach(r => {
        if (r.appCommand) {
          ...
          r.click = (f, p, y) => {
            if (!p) {
              let m = fe();
              if (m) if (p = B[m], p && p.isMinimized()) p.restore();else return;
            }
            p && (!y.triggeredByAccelerator || !y.shiftKey && !y.ctrlKey && !y.metaKey && !y.altKey) && p.webContents.executeJavaScript(`app.commands.executeCommandById(${JSON.stringify(s)})`);
          };
          delete r.appCommand;
        }
...
${s}`);
      ...
      submenu: [{
        ...
        accelerator: v ? "Cmd+Option+Shift+V" : "Shift+CommandOrControl+Alt+V",
      ...
      submenu: [...j(h, () => [{
        ...
        click(n, r) {
          r && r.webContents.executeJavaScript(`
									(() => {
  ...
  function fe() {
    ...
    for (let l in B) {
      let a = B[l];
      a.isDestroyed() || (!e || a.focusTime > e.focusTime) && (e = a, o = l);
    }
    return o;
  }
  function K(e) {
    ...
    let n = () => {
      ...
      a.show();
      let m = o.zoom;
      m && typeof m == "number" && d.executeJavaScript(`require('electron').webFrame.setZoomLevel(${o.zoom})`);
    };

    a.menuBarVisible = false;
    P && !v && U(() => a.setIcon(P));
    function r() {
    ...
    function s() {
      try {
        let m = a.getBounds();
        r() && (o.x = m.x, o.y = m.y, o.width = m.width, o.height = m.height);
    ...
    a.on("closed", () => {
      delete B[e];
      !ue && i.BrowserWindow.getAllWindows().length > 0 && ve(e, false);
    });
    let p;
    function y() {
      clearTimeout(p);
    ...
    return a.on("resize", y), a.on("move", y), a.on("ready-to-show", n), d.on("did-create-window", m => {
      let R = m.webContents;
      ae.set(m, a);
      P && !v && U(() => m.setIcon(P));
      De(m, false);
      f(R);
    }), a.loadURL(Ae).then(n, n), ve(e, true), a;
  }
  function Te(e) {
    ...
    return P && !v && U(() => o.setIcon(P)), ye(o.webContents), o.menuBarVisible = false, o.on("focus", () => {
      te();
    }), o;
  }
  let _;
  ...
  i.app.whenReady().then(() => {
    ...
    i.protocol.registerFileProtocol("app", (t, n) => {
      ...
      r.indexOf("#") > 0 && (r = r.substr(0, r.indexOf("#")));
      r.indexOf(V) === 0 ? (r = decodeURIComponent(r.substr(V.length)), r = D.resolve(D.join(c, r)), r.indexOf(D.resolve(c)) !== 0 && (r = ""), s = true) : r.indexOf(se) === 0 ? (r = decodeURIComponent(r.substr(se.length)), N || (r = "/" + r), r = D.resolve(r), _e(r) && (s = true)) : r = "";
      let f = {};
    ...
    i.ipcMain.on("check-update", (t, n) => {
      n && (b.updateDisabled && u.emit("disable", false), u.emit("check"), b.updateDisabled && setTimeout(() => u.emit("disable", true), 50));
      t.returnValue = E;
    });
    i.ipcMain.on("disable-update", (t, n) => {
      n === true ? (b.updateDisabled = true, u.emit("disable", true), k(), console.log("Updates disabled.")) : n === false && (delete b.updateDisabled, u.emit("disable", false), k(), console.log("Updates enabled."));
      t.returnValue = b.updateDisabled;
    });
    i.ipcMain.on("disable-gpu", (t, n) => {
      n === true ? (b.disableGpu = true, k()) : n === false && (delete b.disableGpu, k());
      t.returnValue = b.disableGpu;
    });
    i.ipcMain.on("insider-build", (t, n) => {
      n === true ? (b.insider = true, u.emit("insider", true), k()) : n === false && (delete b.insider, u.emit("insider", false), k());
    ...
    i.ipcMain.on("print-to-pdf", async (t, n) => {
      ...
      try {
        ...
        let f = await r.printToPDF(n);
        await g.promises.writeFile(s, f);
        n.open && be(s);
      } finally {
        console.log("Done.");
        r.send("print-to-pdf", {});
    ...
    i.ipcMain.on("vault", t => {
      for (let n in B) if (B[n].webContents === t.sender) {
        t.returnValue = {
          id: n,
    ...
    i.ipcMain.on("vault-move", (t, n, r) => {
      if (n && typeof n == "string") for (let s in F) {
        let f = F[s];
        if (f.path === n) {
          if (B[s]) {
    ...
    i.ipcMain.on("request-url", async (t, n, r) => {
      ...
      if (p && R.setHeader("Content-Type", p), m) for (let w in m) try {
        R.setHeader(w, m[w]);
      } catch (S) {
        console.error(S);
      }
      ...
      R.on("response", w => {
        ...
        w.on("end", () => {
          ...
          let me = W.buffer.slice(W.byteOffset, W.byteOffset + W.byteLength);
          t.reply(n, {
            status: w.statusCode,
            headers: w.headers,
            body: me
      ...
      try {
        typeof y == "string" ? R.write(y) : y instanceof ArrayBuffer && R.write(Buffer.from(new Uint8Array(y)));
        R.end();
      } catch (w) {
        t.reply(n, {
          error: w
    ...
    i.ipcMain.on("set-icon", (t, n, r) => {
      b.icon && g.rmSync(D.join(C, b.icon), {
        force: true
      });
      n && r ? g.writeFileSync(D.join(C, n), r) : n = null;
      (b.icon || "") !== (n || "") && (n ? b.icon = n : delete b.icon, k());
      t.returnValue = null;
    });
    ...
    i.ipcMain.on("create-browser-session", async (t, n, r) => {
      ...
      s || (r === true && Z(), s = {
        ...
        adblock: !!r
      }, I[n] = s, s.session.setUserAgent(s.session.getUserAgent().split(" ").filter(f => !/^(obsidian|electron)/i.test(f)).join(" ")), s.session.webRequest.onBeforeRequest({
        urls: ["https://*/*", "http://*/*"]
      }, (f, p) => {
        ...
        for (let m in y) m.toLowerCase() === "sec-fetch-dest" || m.toLowerCase() === "sec-ch-ua" ? delete y[m] : m.toLowerCase() === "user-agent" && f.url.startsWith("https://accounts.google.com/") && (y[m] = "Chrome");
        p({
          requestHeaders: y
        });
      }), s.session.setPermissionCheckHandler((f, p, y) => Oe.includes(p)), s.session.setPermissionRequestHandler((f, p, y, m) => {
        y(Oe.includes(p));
      }), s.session.setDevicePermissionHandler(f => false));
      (r === true || r === false) && (s.adblock = r);
    });
    ...
    e.onBeforeRequest({
      ...
      let p = true;
      f + "/" === V && (p = false);
      f === "null" && r === r.top && s.startsWith(V) && (p = false);
      n({
        cancel: p
      });
    });
    e.onBeforeSendHeaders({
      ...
      for (let s in r) (s.toLowerCase() === "sec-fetch-dest" || s.toLowerCase() === "sec-ch-ua") && delete r[s];
      n({
        requestHeaders: r
      });
    });
    e.onHeadersReceived({
      ...
      try {
        if (!m) {
          let w = y.mainFrame;
          m = w.framesInSubtree.filter(S => S !== w).some(S => S.routingId === p.routingId && S.processId === p.processId);
        }
      } catch (w) {}
      let R = false;
      for (let w in r) {
        w.toLowerCase() === "access-control-allow-origin" && (R = true);
        w.toLowerCase() === "x-frame-options" && delete r[w];
        w.toLowerCase() === "cross-origin-opener-policy" && delete r[w];
        w.toLowerCase() === "content-security-policy" && (r[w] = r[w].map(S => S.replace(/\s*frame-ancestors [^;]*(;|$)/g, "")));
        w.toLowerCase() === "set-cookie" && m && (r[w] = r[w].map(S => /Secure;/i.test(S) ? S.replace(/SameSite=Lax/i, "SameSite=None") : S));
      }
      n({
        responseHeaders: r
      });
    ...
    i.session.defaultSession.setPermissionRequestHandler((t, n, r, s) => {
      ...
      s.isMainFrame && s.requestingUrl === "about:blank" && n.startsWith("clipboard-") && (f = true);
      n === "openExternal" ? f = false : n === "fullscreen" && (f = true);
    ...
    function l(t) {
      if (t && typeof t == "string") {
        ...
        for (let r in F) {
          let s = F[r];
          if (s.path === t) return s.ts = Date.now(), K(r), i.app.addRecentDocument(t), true;
        }
        let n = je(16);
        return F[n] = {
          path: t,
          ts: Date.now()
        }, K(n), i.app.addRecentDocument(t), true;
      }
    ...
    }), v && U(() => i.app.dock.setIcon(P)), i.Menu.setApplicationMenu(ee), i.app.on("web-contents-created", (t, n) => {
      we(n);
      n.noContextMenu = false;
      n.hostWebContents && n.on("context-menu", (r, s) => {
        n.noContextMenu || He(i.BrowserWindow.fromWebContents(n), n, s);
      });
    }), i.app.on("open-file", (t, n) => {
      t.preventDefault();
      let r = D.resolve(n);
      let s = "";
      for (let f in F) {
    ...
    }), Pe(process.argv), ce && he(ce), i.app.on("open-url", (t, n) => {
      t.preventDefault();
      he(n);
    }), Object.keys(B).length === 0) de();else {
      for (let t in F) B[t] || delete F[t].open;
      k();
    }
  });
  !h && !i.app.isDefaultProtocolClient("obsidian") && i.app.setAsDefaultProtocolClient("obsidian");
  let Ie = "obsidian://";
  function he(e) {
    if (!e.startsWith(Ie)) return;
    let o = e;
    console.log("Received callback URL", e);
    e = e.substr(Ie.length);
    let l = {};
    if (e.startsWith("/")) {
      let n = e;
      N && (n = e.substr(1));
      ...
      l.action = "open";
      l.vault = n[0];
      l.file = n.slice(1).join("/");
    } else {
      let n = "";
      let r = "";
      let s = e.indexOf("?");
      let f = e.indexOf("#", Math.max(0, s));
    ...
    } else if (t && typeof t == "string") for (let n in F) {
      let r = F[n].path;
    ...
    a ? Me(a, l) : i.dialog.showErrorBox("Vault not found", "Unable to find a vault for the URL " + o);
  }
  function Me(e, o) {
    let l = K(e);
    let a = JSON.stringify(o);
    let d = `var w=window;if(typeof w.OBS_ACT === "function"){w.OBS_ACT(${a})}else{w.OBS_ACT=${a}}`;
    let t = l.webContents;
    l.loaded ? t.executeJavaScript(d) : t.once("did-finish-load", () => t.executeJavaScript(d));
  }
  function Pe(e) {
    for (let o of e) if (o.startsWith("obsidian://")) return he(o), true;
    return false;
  }
};